...
class Environment(object):
  def __init__(self):
    self.Xtrain, self.S1train, self.S2train, self.ytrain, self.Xtest, self.S1test, self.S2test, self.ytest =\
        process_gridworld_data(input=FLAGS.input, imsize=FLAGS.imsize)

class Agent(object):
  def __init__(self, env):
    self.env = env
    self.X  = tf.placeholder(tf.float32, shape=[None, FLAGS.imsize, FLAGS.imsize, FLAGS.ch_i])
    # symbolic input batches of vertical positions
    self.S1 = tf.placeholder(tf.int32, shape=[None, FLAGS.statebatchsize])
    # symbolic input batches of horizontal positions
    self.S2 = tf.placeholder(tf.int32, shape=[None, FLAGS.statebatchsize])
    self.y  = tf.placeholder(tf.int32, shape=[None])

    self.logits, self.nn = self.VI_Block()
  ...
  def build_optimizer(self):
    opt = tf.train.RMSPropOptimizer(learning_rate = FLAGS.lr, epsilon = 1e-6, centered = True)
